...
describe('sealed-service production deployment closure', () => {
  it('boots from the minimal runtime closure without core or Cordis', async () => {
    ...
    const searchPackage = join(deployment, 'node_modules', '@dsh-rsi', 'search')
    await mkdir(join(searchPackage, 'lib'), { recursive: true })
    await cp(join(repoRoot, 'packages', 'dsh-rsi-sealed-service', 'lib'), join(deployment, 'lib'), {
      recursive: true,
    })
    await Promise.all(
      ['split.js', 'calibration.js', 'rng.js'].map((file) =>
        cp(
          join(repoRoot, 'packages', 'dsh-rsi-search', 'lib', file),
    ...
    await expect(access(join(deployment, 'node_modules', '@dsh-rsi', 'core'))).rejects.toThrow()
    const deployedService = await readFile(join(deployment, 'lib', 'service.js'), 'utf8')
    expect(deployedService).not.toContain('@dsh-rsi/core')
