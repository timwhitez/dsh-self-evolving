...
beforeEach(async () => {
  scratch = await mkdtemp(join(tmpdir(), 'dsh-self-evolving-capsule-'))
})
...
describe('Gate 1 — packed capsule offline boot', () => {
  it(
    ...
    async () => {
      ...
      await expect(
        stat(join(capsuleDir, 'runtime', 'bin', 'dsh-self-evolving-acp')),
      ).resolves.toBeDefined()
      await expect(
        stat(join(capsuleDir, 'runtime', 'dsh-self-evolving-acp')),
      ).resolves.toBeDefined()
      ...
      await mkdir(join(bootDir, 'node_modules', '@dsh-self-evolving'), { recursive: true })
      ...
      await cp(
        ...
        join(bootDir, 'node_modules', '@dsh-self-evolving', 'candidate-baseline'),
        { recursive: true },
      )
      ...
      expect(entries).toContain('self-evolving-candidate:@dsh-self-evolving/candidate-baseline')
  ...
  it(
    ...
    async () => {
      ...
      expect(stderr.join('')).not.toContain('/root/dsh-self-evolving/deepseek-harness')
    },
  )
    ...
    async () => {
      ...
      await writeFile(
        ...
        [
          ...
          'WORKDIR /work',
          'ENTRYPOINT ["/capsule/runtime/dsh-self-evolving-acp"]',
          '',
        ].join('\n'),
      )
      dockerImage = `dsh-self-evolving-gate1-${process.pid}-${Date.now()}`
