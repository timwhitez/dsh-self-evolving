...
async function collectTypedTree(root: string): Promise<Map<string, string>> {
  ...
  async function walk(dir: string): Promise<void> {
    for (const entry of (await readdir(dir, { withFileTypes: true })).sort((left, right) =>
      ...
      if (CONTROL_PATHS.has(rel)) {
        if (!info.isFile() || info.isSymbolicLink() || info.nlink !== 1) {
          throw new Error(`capsule tree: control path must be one regular file: ${rel}`)
        }
        continue
      }
      if (info.isDirectory()) {
      ...
      if (info.isSymbolicLink()) {
        entries.set(`symlink:0777:${rel}`, sha256(await readlink(abs)))
        continue
      }
      if (!info.isFile()) {
...
async function collectLegacyTree(root: string): Promise<Map<string, string>> {
  ...
  async function walk(dir: string): Promise<void> {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if (info.isSymbolicLink()) {
        entries.set(`symlink:${rel}`, sha256(await readlink(abs)))
        continue
      }
...
export async function verifyCapsuleTreeManifest(capsuleRoot: string): Promise<CapsuleTreeResult> {
  const [manifestBytes, sums] = await Promise.all([
    readFile(join(capsuleRoot, 'capsule.json')),
    readFile(join(capsuleRoot, 'SHA256SUMS'), 'utf8'),
  ])
  let manifest: {
