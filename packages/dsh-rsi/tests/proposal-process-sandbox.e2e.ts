...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'dsh-rsi-proposal-sandbox-'))
})
...
describe('Gate 4 — outer proposal process sandbox', () => {
  it('enforces read-only inputs, one writable root, empty secrets, and no network', async () => {
    ...
    const probe = [
      ...
      '})',
      'if (!parentWriteDenied || !hostPathAbsent || !networkDenied || process.env.DEEPSEEK_API_KEY) process.exit(41)',
      "await writeFile('/work/children/result.json', JSON.stringify({ parentWriteDenied, hostPathAbsent, networkDenied }))",
  ...
  it('kills and drains the whole PID namespace on timeout', async () => {
    const paths = await mounts()
    const marker = `dsh-rsi-sandbox-survivor-${process.pid}`
    await writeFile(
