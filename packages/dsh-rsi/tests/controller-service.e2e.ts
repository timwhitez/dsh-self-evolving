...
import * as RsiBundle from '../src/index.js'
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'dsh-rsi-controller-service-'))
})
...
describe('Gate 3 — Cordis controller service lifecycle', () => {
  it('ships a DSH bundle patch that mounts the single controller row', async () => {
    ...
    expect(patch.match(/name: '@dsh-rsi\/core'/g)).toHaveLength(1)
    expect(patch).toContain('DSH_RSI_STATE_DIR')
    expect(patch).toContain('DSH_RSI_RUN_ID')
  })

  it('provides only ctx.rsi, durably records state, and releases its writer lock on unload', async () => {
    ...
    const service = ctx.get<RsiBundle.RsiService>('rsi')
    expect(service).toBeDefined()
    expect(RsiBundle.name).toBe('dsh-rsi')
    ...
    expect(ctx.get('rsi')).toBeUndefined()
    ...
    expect((await ctx.get<RsiBundle.RsiService>('rsi')!.status()).eventCount).toBe(17)
    await ctx.fiber.dispose()
    ctx = undefined
